...
struct node {
    ...
    bool operator==(const node& cr) const {
        return (n_x == cr.n_x and n_y == cr.n_y and n_idx == cr.n_idx and
                n_lt == cr.n_lt);
    }
    void n_print() { cout << "node (x:" << n_x << ",y:" << n_y << ")"; }
};
struct vec {
    //二维坐标系向量
    ...
    vec(double cx = 0, double cy = 0) : v_x(cx), v_y(cy) {}
    vec(const node& cbeg, const node& cend)
        // cbeg为向量起点，cend为向量终点
        : v_x(cend.n_x - cbeg.n_x), v_y(cend.n_y - cbeg.n_y) {}
    vec(const vec& cr) : v_x(cr.v_x), v_y(cr.v_y) {}
};
struct segment {
    //二维坐标系线段
    ...
    segment() : s_lt(), s_rt() {}
    segment(const node& cl, const node& cr) : s_lt(cl), s_rt(cr) {}
    segment(const segment& cs) : s_lt(cs.s_lt), s_rt(cs.s_rt) {}
    void s_print() {
        cout << "segment  left ";
        s_lt.n_print();
        cout << "  right ";
