...
pair<bbs_node, bbs_node> bbs_invalid_pair(bbs_node(-1, -1, -1, -1), bbs_node(-1, -1, -1, -1));
...
{//对q_expand队列的头节点的所有邻节点进行扩展
 ...
 //若已经存在 则q_expand的头节点与q_exist中该邻节点是相邻的 即两队列相遇
	...
	if(p.m_row + 1 < m && !visited[p.m_row + 1][p.m_col]){
		if((pos = find(q_exist.begin(), q_exist.end(), s[p.m_row + 1][p.m_col])) !=
					q_exist.end())
			return(pair<bbs_node, bbs_node>(p, *pos));
		s[p.m_row + 1][p.m_col].m_father_row = p.m_row;
		s[p.m_row + 1][p.m_col].m_father_col = p.m_col;
	...
	if(p.m_col - 1 >= 0 && !visited[p.m_row][p.m_col - 1]){
		if((pos = find(q_exist.begin(), q_exist.end(), s[p.m_row][p.m_col - 1])) !=
					q_exist.end())
			return(pair<bbs_node, bbs_node>(p, *pos));
...
{//从点b向起点反向输出路径
 //注意这里递归的使用 输出的顺序是从起点到终点的顺序
	if(b.m_father_row != -1 && b.m_father_col != -1)
		bidirection_breadth_search_print_backward(s[b.m_father_row][b.m_father_col], s);		
...
{//meet_pos中是两队列相遇处的两节点
 ...
 //输出路径时向后递归逆序输出q_beg的路径 向前顺序输出q_end的路径
	bidirection_breadth_search_print_backward(s[meet_pos.first.m_row][meet_pos.first.m_col], s);
