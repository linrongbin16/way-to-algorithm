...
//原题目是游荡的奶牛(奶牛历来是usaco中的终极杀手)

//最直觉的方法是：对于每个节点可以进行一次限制步数为level的遍历
...
//
//对于节点5，假设它的奶牛能游荡的最大距离level是 2，那么节点5：
...
//当节点i向上 1 层到达父节点，再向下 1 层可以到达它的兄弟节点
//这时它的剩余步数是 level-2
//例如图中节点5向上1层到达 2，再向下1层到达它的兄弟节点4
//这时它的剩余步数是 level-2，它可以到达兄弟节点4向下 level-2 层的所有子树节点
//则节点i向上 k 层，可以到达它的k祖父节点的 level-k 层所有子树节点
//其中0<=k<=level
...
//对于每个节点i，都可以得到它的孩子们的权值之和，即下 1 层的子树的权值之和
//想要得到节点i下 level 层子树的节点权值之和，可以从下向上动态规划：
//设fdown[i][j]指代节点i向下 j 层所到达的所有节点的权值之和，不包括自己的权值
//fdown[i][1]指代节点i向下 1 层的权值之和
//即为节点i的所有孩子节点的权值之和(向下第 1 层子树节点)，但不包括节点i自己的权值
...
//节点i的向下 j 层的最大权值
//等于它的所有孩子节点k的向下 j-1 层的最大权值之和，再加k自己的权值v[k]
//其中k枚举节点i的所有孩子节点
//其中j枚举所有层数，1 <= j <= level
//
//(ii)再次遍历这棵树，对于每个节点，向上考虑它从第 1 层到第 level 层的祖父节点
//对于节点i向上第 k 层的祖父节点
//节点i可以到达这个祖父节点的向下 level-k 层所有子树节点
//对于每个祖父节点的向下的权值，之前第1遍的动态规划已经解决了
//节点i的向上所能到达的所有节点权值之和就是所有向上第 k 层祖父节点
//的向下 level-k 层的最大权值之和(从向上第1层累加到第 level 层)
//需要注意的问题是：只选取这些祖父节点的兄弟分支，要除掉自己的分支
//设fup[i][j]指代节点i向上第 j 层的那个祖父节点的所有孩子节点(除过自己这一分支)
