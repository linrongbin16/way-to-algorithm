...
{//二叉树根节点为r 其中每个节点的权值数组t_f初始化为0 权值为t_value
 //二叉树保留resv个节点 返回二叉树最大权值
	...
	for(int i = 0; i < resv; ++i){
		int lmax = binary_tree1(r->m_leftchild, i);
		int rmax = binary_tree1(r->m_rightchild, resv - i - 1);
		...
		r->m_f[resv] = max(r->m_f[resv], lmax + rmax);
	}
	//对于以r为根节点的树
...
{//初始时二维数组f中所有成员均为0
	...
	for (int i = 0; i < resv; ++i) {
		int lmax = binary_tree2(r->m_leftchild, f, i);
		int rmax = binary_tree2(r->m_rightchild, f, resv - i - 1);
		f[r->m_index][resv] = max( f[r->m_index][resv], lmax + rmax );
	}
