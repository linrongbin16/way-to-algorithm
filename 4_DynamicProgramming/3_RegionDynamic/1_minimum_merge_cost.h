...
//对于序列s的从1到n这n个元素
//设summ[i][j]指代从i到j元素之和 设f[i][j]指代合并i到j的最小代价
//我们现在考虑最后一次合并:
//假设合并从1到i和从i+1到n这两堆元素
//从1到i的最小代价为f[1][i] 从i+1到n的最小代价为f[i+1][n]
//还需要加上summ[1][i]和summ[i+1][n] 
//所以最后一次合并f[1][n] = f[1][i] + f[i+1][n] + summ[1][i] + summ[i+1][n]
//只要枚举所有的i 其中1<i<n 就可以求出最后一步的最小代价
//由此递推回去 容易得出整个过程的状态转移方程是:
//f[i][j] = min( f[i][k] + f[k+1][j] + summ[i][k] + summ[k+1][j] )
//其中1<=i<k<j<=n
...
int minimum_merge_cost(int s[MAX], int n)
{
	...
	int f[MAX + 1][MAX + 1];
    int summ[MAX + 1][MAX + 1];
    //对于1<=i<=n f[i][i]=0 summ[i][i]=0
	...
	for (int i = 1; i <= n; ++i)
		for (int j = 1; j <= n; ++j)
			for (int k = i; k <= j; ++k)
				//summ[i][j]存储序列i到j的元素之和
				summ[i][j] += s[k];
    //对于相邻的1<=i<i+1<=n f[i][i+1]=summ[i][i]+summ[i+1][i+1]
    for (int i = 1; i <= n; ++i) {
       f[i][i+1] = summ[i][i] + summ[i+1][i+1];
    }
    //1<=k<n 计算以k为距离的s[i]和s[i+k]之间的代价f[i][i+k]
    for (int k = 1; k < n; ++k)
        for (int i = 1; i+k <= n; ++i)
            //j是[i,i+k]之间的所有中间元素
            for (int j = i+k; j <= i+k; ++j)
				f[i][i+k] = min( f[i][i+k],
						f[i][j] + f[j][i+k] + summ[i][j] + summ[j][i+k] );
