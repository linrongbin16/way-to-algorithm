...
//然后每次深度优先搜索遍历到的所有节点属于一个强连通分支
//而且Kosraju算法有一个副作用
//即第二次通过dfs遍历有向图G时，遍历节点的顺序也是该节点在强连通分支中的拓扑顺序
//
//由于强连通分支一定存在环，因此这个拓扑排序只能使用dfs方法的拓扑排序代码
...
{//图G有g_l.size()个节点，下标从0到g_l.size()-1
 ...
 //属于同一强连通分支的节点其g_value值相等
	...
	//一次dfs遍历到的所有节点属于同一个强连通分支，用grp标记分支号
...
{//与深度优先搜索中的dfs_kosaraju函数遍历方法一样
 //但不记录遍历的节点数量，而且visited数组用grp的值标记
	//用grp值来标记visited数组，使得不同的强连通分支的visited值不一样
	visited[p] = 1;
