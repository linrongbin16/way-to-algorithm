...
$$ (2) $$ 在构造前缀树时我们已知$$ pt[2 \dots 9] = pt[7 \dots 12] $$，$$ pt[2 \dots 9] $$的长度为$$ 3 $$。那么我们可以从前缀树中字符串的末尾字符位置$$ pt[12] $$直接跳到$$ pt[9] $$。这时必然有$$ text[1 \dots 3] = pt[2 \dots 9] $$，继续向右匹配发现$$ text[4] = pt[11] $$，可知$$ text[1 \dots 4] = pt[2 \dots 11] $$。图中红色的连线称为后缀链接/后缀指针$$ suffix link $$，也称失败链接/失败指针$$ failure link $$；

后缀指针是前缀树中从$$ pt[i] $$指向$$ pt[j] $$的链接，两节点分别是字符串$$ \alpha $$和$$ \beta $$的末尾字符（注意$$ \alpha, \beta $$不在同一个字符串中，也并不一定是前缀树中的字符串，子字符串即可），且$$ \beta $$是前缀树中$$ \alpha $$的最长后缀字符串（后缀字符串是非空的，且不等于原字符串）。比如下图中$$ pt[2 \dots 9] = "oar" $$是$$ pt[3 \dots 12] = "soar" $$的后缀字符串：

![AhoCorasick6.svg](../res/AhoCorasick6.svg)

当文本匹配到前缀树某个位置失败，且该位置上没有后缀指针时，我们从前缀树的根节点开始重新匹配。这可以看作是特殊的空后缀指针，如图所示：

![AhoCorasick7.svg](../res/AhoCorasick7.svg)

当我们在前缀树上匹配结束时（成功或失败都是匹配结束），都可以沿着失败指针找到下一个后缀继续匹配，通过失败指针可以跳过之前已经成功匹配的字符，降低时间复杂度。沿着失败指针的跳转可以是多次的，直到再也没有失败指针为止。失败指针的优化可以避免重复搜索前缀树，时间复杂度降低到$$ O_{3} = O(\sum_{i=1}^{k} m_{i}) + O(n) $$。
...
我们在前缀树上建立输出指针来优化，下图中蓝色线从节点$$ pt[i] $$指向节点$$ pt[j] $$，其中以$$ pt[j] $$为末尾字符的字符串是前缀树中的字符串（红色节点），两节点分别是字符串$$ \alpha $$和$$ \beta $$的末尾字符，且$$ \beta $$是$$ \alpha $$的后缀字符串。匹配文本时，若当前节点上有输出指针，可以直接沿着输出指针成功匹配，并且递归的一直沿着输出指针成功匹配，直到没有输出指针为止。同一个节点上可以有多个输出指针，指向多个前缀树中的字符串。

![AhoCorasick9.svg](../res/AhoCorasick9.svg)

最终得到Aho Corasick算法：对于文本$$ text $$上的任意字符$$ text[i] $$，从前缀树$$ pt $$的根部开始匹配：

$$ (1) $$ 当匹配结束时，若$$ text $$上的一部分与前缀树的字符串$$ pattern_{i} $$匹配，则成功匹配；

$$ (2) $$ 匹配结束时（成功或失败都算匹配结束），若前缀树当前位置上有失败指针，则沿着失败指针继续匹配，并且递归的继续沿着所有可用的失败指针尝试继续匹配，直到没有失败指针；

$$ (3) $$ 匹配中若遇到输出指针，则沿着输出指针找出所有成功的匹配，并且递归的沿着所有输出指针匹配出所有前缀树中的字符串，直到没有输出指针；

#### 构建AC自动机

构建AC自动机需要三步：构建前缀树；构建后缀指针；构建输出指针。

前缀树的构造详见本书的DataStructure-PrefixTree。

后缀指针的构造实际是求$$ pattern_{i} $$的最长后缀字符串$$ pattern_{j} $$。若$$ x $$是$$ w $$的后缀字符串，显然在$$ x, w $$后面同时增加一个字符$$ a $$，使得$$ xa $$为$$ wa $$的最长后缀字符串。
