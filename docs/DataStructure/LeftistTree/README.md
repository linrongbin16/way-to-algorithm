...
$$ (1) $$ 递归向下合并子树的根节点，合并两个左偏树，每次合并时比较两个子树根节点的值$$ value_{a} $$和$$ value_{b} $$，总是用根节点较大的子树$$ value_{b} $$（设$$ value_{b} \gt value_{a} $$）替换根节点较小的子树$$ value_{a} $$的右孩子节点$$ value_{a-left} $$，替换后$$ value_{a-left} $$成为新的子树。然后递归的，继续考虑$$ value_{a-left} $$作为根节点的子树与$$ value_{b} $$作为根节点的子树；

$$ (2) $$ 递归向上更新所有被修改过的节点的距离；
