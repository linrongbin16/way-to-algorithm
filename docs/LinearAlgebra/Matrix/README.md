...
$$ n $$阶矩阵的主对角线：即矩阵$$ A_{n,n} $$上的元素$$ a(i,i) $$，其中$$ 1 \leq i \leq n $$。
...
转置矩阵：矩阵$$ A $$的转置矩阵是$$ m \times n $$的矩阵$$ AT $$，$$ AT $$中的任意元素$$ at(j,i) = a(i,j) $$。
...
逆矩阵：$$ n $$阶矩阵$$ A $$存在一个对应的$$ n $$阶矩阵$$ B $$，使得$$ AB = BA = E $$，则称$$ A $$是可逆矩阵，$$ B $$为$$ A $$的逆矩阵。

正定矩阵：设$$ A $$是$$ n $$阶矩阵，$$ x $$是任意$$ n $$维向量，$$ xt $$是$$ x $$的转置，若对于任意的$$ x \neq 0 $$都有$$ xt \times A \times x \gt 0 $$，则称$$ A $$是正定矩阵。

#### 矩阵运算

矩阵加减：能够进行加减运算的两矩阵必须有相等的行和列，$$ m \times n $$的矩阵$$ A + B = C $$，或$$ A - B = C $$。
...
矩阵数乘：矩阵$$ A $$与常数$$ c $$相乘得到矩阵$$ B = A \times c $$，矩阵$$ B $$中的任意元素满足

$$
b_{i,j} = c \times a_{i,j}
$$

矩阵乘：能够进行乘运算的矩阵$$ A $$和$$ B $$必须$$ A $$的列与$$ B $$的行相等，即$$ A $$是$$ m \times s $$的矩阵，$$ B $$是$$ s \times n $$的矩阵，$$ A \times B = C $$得到的矩阵$$ C $$是$$ m \times n $$矩阵。矩阵$$ C $$中任意元素满足

$$
c_{i,j} = a{i,1} \times b_{1,j} + a_{i,2} \times b_{2,j} + \dots + a_{i,s} \times b_{s,j}
$$
