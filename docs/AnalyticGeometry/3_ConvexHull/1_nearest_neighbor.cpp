...
double nearest_neighbor(node *p, int n) { //一组顶点p有n个顶点 下标从0到n-1
  node q[MAX];
  sort(p, p + n, compare_x);
  return (div_and_con(p, q, 0, n - 1));
}
double div_and_con(node *p, node *q, int beg,
                   int end) { //点集p有end-beg+1个节点 [beg, end]为左闭右闭区间
                              // beg是子区间中最左下角的顶点下标
                              // end是子区间中最右上角的顶点下标 q是顶点数组
                              // 用于支持计算垂直线两侧的点的距离的操作
  //递归终止条件
  if (beg + 1 == end)
  ...
  int mid = (beg + end) / 2;
  double dist =
      min(div_and_con(p, q, beg, mid), div_and_con(p, q, mid + 1, end));
  int n(0);
...
bool compare_x(node a, node b) { //按x坐标从小到大排序
                                 //若x坐标相等则按y坐标从小到大排序
  if (a.n_x != b.n_x)
    return (a.n_x < b.n_x);
  return (a.n_y < b.n_y);
}
bool compare_y(node a, node b) { //按y坐标从小到大排序
  return (a.n_y < b.n_y);
}
