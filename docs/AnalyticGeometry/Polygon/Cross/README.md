...
向量$$ \vec{v} = [x, y, z] $$表示三维空间中一个有长度的方向，单位长度为$$ 1 $$。也可以写作：
...
$$ (1) $$ 若$$ C \gt 0 $$则$$ \vec{v_{1}} $$在$$ \vec{v_{2}} $$的顺时针方向（按右手螺旋法则从$$ \vec{v_{2}}$$旋转到$$ \vec{v_{1}} $$时，大拇指与单位向量$$ \vec{k} $$方向相反）；

$$ (2) $$ 若$$ C \lt 0 $$则$$ \vec{v_{1}} $$在$$ \vec{v_{2}} $$的逆时针方向（按右手螺旋法则从$$ \vec{v_{2}}$$旋转到$$ \vec{v_{1}} $$时，大拇指与单位向量$$ \vec{k} $$方向相同）；

$$ (3) $$ 若$$ C = 0 $$则$$ \vec{v_{1}} $$与$$ \vec{v_{2}} $$共线，属于边界情况。
