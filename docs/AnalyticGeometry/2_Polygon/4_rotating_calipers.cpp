...
double rotating_calipers(node *s,
                         int n) { //凸多边形s有n个顶点 下标从0到n-1
                                  //顶点按照逆时针方向排列 返回凸多边形的直径
  ...
  //这样设置两向量的方向可以保证向量每次旋转的角度都小于180度
  //也可设置为相反的方向
  vec vl(1, 0), vh(-1, 0);
  //凸多边形直径
  double diam(0);
...
bool get_back(node *s, int n, int low, int high, int l,
              int h) { //旋转的顶点l和h只需要旋转半圈
                       //顶点l回到h的起始位置high
                       //顶点h回到l的起始位置low
  if (s[h] == s[low])
    return (true);
...
void rotate(node *s, int n, int &l, int &h, vec &vl,
            vec &vh) { //将向量vl和vh逆时针旋转 顶点l和h也相应的进行挪动
...
double cosine_c(node a, node b, node c) { //计算三角形abc中顶点c的角度cos值
