...
int ray(node *s, int n,
        node p) {  //多边形s有n个顶点，下标从0到n-1，顶点按照逆时针排列
                   //判断点p是否在多边形s内
                   //若p点在多边形内返回0，若在多边形上返回1，若在多边形外返回2
    //预判断
    if (faraway(s, n, p)) return (2);
...
bool faraway(node *s, int n, node p) {  //判断点p是否在多边形区域外
...
double node2segment_distance(segment l,
                             node p) {  //计算点p到线段l所在直线的距离
