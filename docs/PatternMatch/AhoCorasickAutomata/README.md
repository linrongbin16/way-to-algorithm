...
在文本$$ text $$中查找$$ k $$个模式$$ pattern $$出现的所有位置。其中$$ text $$长度为$$ n $$，$$ pattern_{i} $$的长度为$$ m_{i} $$，其中最长的模式长度为$$ m_{max} $$且$$ n \gt m_{max} $$，所有模式长度之和为$$ m_{sum} = \sum_{i=1}^{k} m_{i} $$，且所有模式中没有两两重复的模式。
...
我们能否在一次匹配$$ text $$的过程中就同时找出所有模式呢？即并行算法（算法上并行，而非多线程/多进程上并行）。
...
构建前缀树的时间复杂度为$$ O(m_{sum}) $$，之后对于文本上每个字符，都匹配一次前缀树即可，该算法的时间复杂度为$$ O(m_{sum}) + O(n \cdot max(m_{i})) $$。当$$ n $$远大于$$ max(m_{i}) $$时显然构造第二种算法更优。
