...
对于相同符号且满足$$ a \ge b $$的减法$$ c = a - b $$，设$$ a[0], b[0] $$为数字的最低位（个位），用下标$$ i $$从最低位到最高位开始遍历$$ a, b $$的每一位（$$ i = 0 \rightarrow max(n_{a}, n_{b}) + 1 $$）。初始时设$$ c $$的每一位都为$$ 0 $$。则有：
...
\begin{matrix}
c[10^j + i] = ( c[10^j + i] + a[i] \times b[j] ) % 10   \\
c[10^j + i+1] = c[10^j + i+1] + ( c[10^j + i] + a[i] \times b[j] ) \div 10
\end{matrix}
