* [Upper Folder - 上一级目录](../)
* [Source Code - 源码](https://github.com/zhaochenyou/Way-to-Algorithm/blob/master/src/Calculation/LargeNumber.hpp)
* [Test Code - 测试](https://github.com/zhaochenyou/Way-to-Algorithm/blob/master/src/Calculation/LargeNumber.cpp)
...
<div>
问题：
<p id="i">分别计算两个整数\(a\)和\(b\)，两个小数\(c\)和\(d\)的加减乘除，这些数字非常大，无法用原生的\(int64\)、\(float64\)（\(double\)）来存储。</p>
解法：
<p id="i">巨大的整数可以通过数组来存储，然后模拟加减乘除四则运算的算法，来得到结果。 </p>
