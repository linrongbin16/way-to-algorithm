...
(2) 代码第3-4行：比较$$ s[i] $$和$$ s[i+1] $$，若$$ s[i] \gt s[i+1] $$则交换两个元素，否则不做任何操作。这样一次操作会将$$ left $$中的最大元素移动到最右边，下一次调用`travel_left`函数时，只需要将输入参数$$ k $$变为$$ k - 1 $$，就可以将上一次$$ left $$最右边的元素加入$$ right $$的最左边。如图；
