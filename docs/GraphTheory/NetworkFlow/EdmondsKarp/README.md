...
$$ (2) $$ 当$$ queue $$不为空时，从中取出头节点$$ v_i $$，若该节点是汇点$$ v_i = t $$则已经找到一条从源点到汇点的增广路径；若该节点不是汇点则遍历其所有邻节点找出所有的未被染红且剩余容量$$ c(i,j) \gt 0 $$的邻节点$$ v_j $$，将其加入队列$$ queue $$中，染红，并设置逆向指针$$ from(j) = i $$。这样重复，若最终搜索到汇点$$ t $$则找到一条增广路径，沿着逆向指针可以得到该增广路径上的所有边；若直到队列$$ queue $$为空也没有搜索到汇点$$ t $$则说明无法再找到更多的增广路径；
...
可以发现，每次找出一条增广路径后，剩余网络中至少有一条边的剩余容量变为$$ 0 $$，上图中边$$ e_{0,1} $$的剩余容量$$ c(0,1) = 0 $$。
