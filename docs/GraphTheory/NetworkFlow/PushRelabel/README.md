...
设网络中每个节点都有一个水位高度$$ level $$，设节点$$ v_i $$的邻节点为$$ [v_{1}, v_{2}, \dots, v_{n}] $$，与这些邻节点的边为$$ e_{i,j} $$（其中$$ 1 \leq j \leq n $$），容量为$$ c(i,j) $$。节点$$ v_i, v_j $$已经存在的流为$$ f(i,j) $$，满足反对称性$$ f(i,j) = -f(j,i) $$，当$$ f(i,j) \lt c(i,j) $$时边$$ e_{i,j} $$仍然可以容纳更多的流，当$$ f(i,j) = c(i,j) $$时边$$ e_{i,j} $$为饱和边，不能容纳更多的流。
...
$$ (2) $$ 节点$$ v_i $$存在邻节点$$ v_j $$可以容纳额外的流（即$$ c(i,j) - f(i,j) \gt 0 $$），且水位高度之差满足$$ level_{i} \leq level_{j} $$；
