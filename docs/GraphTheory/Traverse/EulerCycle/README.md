...
本文介绍求无向图欧拉回路的$$ Fleury $$算法。我们假定本问题给定的无向图$$ UG $$中必然存在欧拉回路（因为欧拉回路存在的判定非常简单）。设矩阵$$ g $$表示无向图$$ UG $$，其中$$ g[i,j] = 1 $$表示顶点$$ v_i $$到$$ v_j $$之间存在单向边$$ e_{i,j} $$，$$ g[i,j] = 0 $$表示顶点$$ v_i $$到$$ v_j $$之间不存在单向边$$ e_{i,j} $$。
