...
埃氏筛选中对某些素数重复筛选导致时间复杂度增加，反过来只要对每个整数只筛选一次，则算法的时间复杂度会降低到$$ O(n) $$。欧拉筛法通过筛选所有合数，找出$$ [2, n-1] $$中的素数。

设合数：

$$
n = p_{0}^{k_{0}} \cdot p_{1}^{k_{1}} \cdots p_{m-1}^{k_{m-1}}
$$

其中$$ p_{i} $$为素数（$$ 0 \leq i \lt m $$），根据合数定义可知至少存在两个素数因子，即$$ m \geq 2, k_{i} \geq 1 $$。
