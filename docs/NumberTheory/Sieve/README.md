...
按照素数的定理，判断一个正整数$$ x $$是否为素数，需要遍历$$ [1 \dots x] $$中所有数字$$ i $$是否能被$$ x $$整除，即$$ x % i = 0 $$。判断一个数字的时间复杂度为$$ O(n) $$，判断$$ n $$个数字的时间复杂度为$$ O(n ^ 2) $$。埃拉托斯特尼筛选法（Eratosthenes Sieve）可以在$$ O(n) $$的时间复杂度内完成所有判断。

设置数组$$ s = [1 \dots n] $$，$$ s[i] $$表示$$ i $$是否为素数。初始时显然有$$ s[1] = false $$。
...
因为显然偶数中除了$$ 2 $$都是合数，可以跳过所有偶数，只考察奇数。
