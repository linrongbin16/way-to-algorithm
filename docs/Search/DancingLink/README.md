...
舞蹈链算法用十字链表来将所有元素串联起来，节点$$ [x, y] $$的表示$$ x \in s $$，$$ x \in s_{y} $$。沿着十字链表的行可以遍历子集$$ s_{y} $$的所有元素，沿着十字链表的列可以遍历所有包含$$ x $$的子集。
...
如果尝试了所有的子集组合仍无法找出精确覆盖，则说明该条件下不存在精确覆盖。舞蹈链算法的时间复杂度与递归的时间复杂度一样是$$ O(n^m) $$。
