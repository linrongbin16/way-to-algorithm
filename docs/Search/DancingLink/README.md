...
集合$$ s = [x_{1}, x_{2}, \dots , x_{m}] $$拥有$$ m $$个成员，有$$ n $$个子集$$ t = [ s_{1},s_{2}, \dots ,s_{n} ] $$，其中每个子集$$ s_{i} $$拥有一部分$$ s $$中的成员，数量为$$ n_{i} $$（$$ 1 \le i \le n, 0 \le n_{i} \le m $$）。

在$$ t $$中选择一些子集，组成集合$$ p = [ s_{1}, s_{2}, \dots ] $$，使$$ p $$中的子集所包含的成员可以覆盖集合$$ s $$。设$$ p $$中所有子集包含的成员组成的集合为$$ q $$（即对于$$ \forall x \in q $$，都存在$$ \exists s_{i} \in p $$使得$$ x \in s_{i} $$），则对于$$ \forall x \in s $$，都存在$$ \exists s_{i} \in p $$使得$$ x \in s_{i} $$。
