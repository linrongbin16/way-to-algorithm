...
集合$$ s = [x_{1}, x_{2}, \dots , x_{n}] $$拥有$$ n $$个成员，有$$ m $$个子集$$ t = [ s_{1},s_{2}, \dots ,s_{m} ] $$，其中子集$$ s_{i} $$拥有成员的数量为$$ n_{i} $$（$$ 1 \le i \le m, 0 \le n_{i} \le n $$）。
...
舞蹈链算法用十字链表这种特别的数据结构将所有元素串联起来，坐标为$$ [i, j] $$的节点下标是$$ i \times n + j $$，表示$$ j \in s, j \in s_{i} $$。沿着十字链表的行可以遍历子集$$ s_{i} $$的所有元素，沿着十字链表的列可以遍历所有包含$$ j $$的子集。
