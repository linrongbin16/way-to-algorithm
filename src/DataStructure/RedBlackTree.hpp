...
auto RedBlackTreeFree(RedBlackTree *t) -> void { detail::NodeFree(t->root); }
...
auto RotateLeft(RedBlackTree *t, Node *e) -> void {
    ...
    if (detail::Left(q)) detail::Left(q)->father = p;
    q->left = p;
}
...
auto SwapIndex(Node *a, Node *b) -> void { std::swap(a->index, b->index); }

auto SwapColor(Node *a, Node *b) -> void { std::swap(a->color, b->color); }

auto Color(Node *e) -> char { return e ? e->color : BLACK; }
...
auto Father(Node *e) -> Node * { return e ? e->father : nullptr; }

auto Uncle(Node *e) -> Node * {
    if (!GrandFather(e)) return nullptr;
    if (GrandFather(e)->left == e) return GrandFather(e)->right;
    if (GrandFather(e)->right == e) return GrandFather(e)->left;
    return nullptr;
}

Node *GrandFather(Node *e) {
    if (!e) return nullptr;
    if (!e->father) return nullptr;
    return e->father->father;
}
Node *Brother(Node *e) {
    if (!e) return nullptr;
    if (!e->father) return nullptr;
    if (Left(Father(e)) == e) return Right(Father(e));
    if (Right(Father(e)) == e) return Left(Father(e));
    return nullptr;
}
bool IsLeftChild(Node *e) {
    if (!Father(e)) return false;
...
Node *Left(Node *e) { return e ? e->left : nullptr; }
Node *Right(Node *e) { return e ? e->right : nullptr; }
