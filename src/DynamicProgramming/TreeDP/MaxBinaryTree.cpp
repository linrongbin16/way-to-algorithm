...
static int MaxBinaryTreeRec(BiNode *t, int **f, int n, int m, int j) {
  if (t == NULL) {
    return 0;
  }
  ...
  for (int p = 0; p < j; p++) {
    int l = MaxBinaryTreeRec(t->left, f, n, m, p);
    int r = MaxBinaryTreeRec(t->right, f, n, m, j - p - 1);
...
int MaxBinaryTree(BiNode *t, int n, int m) {
  ...
  int result = MaxBinaryTreeRec(t, f, n, m, m);
  Array2DFree(f, n + 1);
  return result;
}
