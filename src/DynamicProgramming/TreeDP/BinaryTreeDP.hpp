...
//我认为它更加完整的体现了树型动态规划中选择空间的分层本质
//我本来准备用这样的数据结构完成本小节的所有算法
//可是在后面的树型动归问题中 这样的数据结构成为了函数接口的累赘 导致程序难以调试
//所以只用它实现本节中最简单的二叉树动规 后面不再使用这种复杂的数据结构
...
int binary_tree1(binary_tree_node1* r, int resv)
{
    ...
    if (r->m_f[resv]) return r->m_f[resv];

    //枚举左子树和右子树保留节点的个数0<=i<resv
    //左子树分配剩余的i个节点
...
int binary_tree2(binary_tree_node2* r, int f[MAX][MAX], int resv)
{
    ...
    if (f[r->m_index][resv]) return f[r->m_index][resv];
