...
//
// ii)节点i不是树的根节点即不满足low[i] =
// dfn[i]，设节点i的根节点(或i的祖先节点)是k 且节点i存在一个这样的孩子节点j
//从j往后进行dfs遍历不会回到i的根节点(或i的祖先节点)k，至多回到节点i自己
//因此节点i是节点j往后的连通分支和根节点k的唯一连通节点，注意节点i，k，j三节点连通
//该条件表示为dfn[i] <=
//low[j]，即从节点j往后的所有节点中最小的dfn值至多是i的dfn值
//
//对于节点i和它的一个孩子节点(邻节点)j有边e(i,
//j)，若该边为割边(桥)当满足以下条件： dfn[i] <
// low[j]，即从节点j出发往后dfs，遍历到的节点的dfn值最小也比i的dfn值大
...
void dfs_cut(graph_list &g, int p, int *visited, int &time, int *dfn, int *low,
             int father, int root, int &root_son,
             vector<pair<int, int>> &cut_edge);

void cut(graph_list &g,
         vector<pair<int, int>>
             &cut_edge) { //无向图G有g_l.size()个节点，下标从0到g_l.size()-1
...
void dfs_cut(graph_list &g, int p, int *visited, int &time, int *dfn, int *low,
  ...
  for (int i = 1; i < (int)g.g_l[p].size(); ++i) {
    if (visited[g.g_l[p][i].g_idx] == 0) {
      dfs_cut(g, g.g_l[p][i].g_idx, visited, time, dfn, low, p, root, root_son,
              cut_edge);
