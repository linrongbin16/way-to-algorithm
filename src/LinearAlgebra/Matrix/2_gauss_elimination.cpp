...
//|.....										  | ...
//| |0	   0	   0	  ...  c(r,r) ...  c(r,n) | br  | |0	   0	   0
//......		   0	  | br+1| |0	   ......
//0	  | 0	|
//|.....										  | ...
//| 上式中r为方程组的秩，n-r个无效的方程被消去，只留下r个有效方程 1)无解情况
...
// c(1,1)*x1 + c(1,2)*x2 + ... + c(1,r)*xr = b1 - c(1,r+1)*xr+1 - ... -
// c(1,n)*xn 			  c(2,2)*x2 + ... + c(2,r)*xr = b2 - c(2,r+1)*xr+1 - ... - c(2,n)*xn
//......
//								c(r,r)*xr = br - c(r,r+1)*xr+1
//- ... - c(r,n)*xn 当方程组的秩r < n，方程组有无穷解 当方程组的秩r =
//n，方程组有唯一解，有效方程组经过高斯消元之后是这样的： |c(1,1)  0	   0
//...  0	  | b1  | |0	   c(2,2)  0	  ...  0	  | b2  | |0
//0	   c(3,3) ...  0	  | b3  |
//|.....							  | ... |
//|0	   0	   0	  ...  c(r,r) | br  |
//其中的r = n
...
//由于篇幅限制本文不再详述
//更多内容见“数学复习全书(2013年李永乐李正元考研数学
//数学一)”，作者“李永乐”“李正元”
//
//本文引用了“C#算法应用之高斯消元法实现”，作者“佚名”
//“高斯消元法(Gauss Elimination)分析&题解&模板——czyuan原创”，作者“Keep Moving
//永不止步...”
...
void gauss_elimination(
    matrix e, int n,
    double *
        x) { //满秩的增广矩阵e是n行n+1列的，行下标从0到n-1，列下标从0到n
...
void elimination(
    matrix &e,
    int n) { //增广矩阵e是n行n+1列的，最后一列是系数向量b
...
double
select_pivot(matrix e, int n, int k,
             int &index) { //增广矩阵e是n行n+1列的，对k行k列进行选择主元的操作
...
void backward_substitution(
    matrix e, int n,
    double *x) { //秩为n的方程组对应的增广矩阵e是n行n+1列的，最后一列是系数向量b
                 ...
                 //......
                 //| 0		 0		 0		 0	     ...
                 //a(n,n) | bn | 由最后一行可以直接通过a(n,n)*xn = bn计算出xn =
                 //bn / a(n,n) 然后向上依次代入每个方程即可求出x的唯一解x = (x1,
                 //x2, x3, ..., xn) 这个求解过程称为逆向代入
  for (int i = n - 1; i >= 0; --i) {
    //满秩方程组有唯一解的情况中
...
void forward_substitution(
    ...
    double *x) { //秩为n的方程组对应的增广矩阵e是n行n+1列的，最后一列是系数向量b
                 ...
                 //| a(1,1)	 0		 0		 0	   	 ...
                 //0	   | b1 | | a(2,1)  a(2,2)  0		 0
                 //...	0	   | b2 | | a(3,1)  a(3,2)  a(3,3)  0       ...
                 //0	   | b3 |
                 //......
                 //| a(n,1)  a(n,2)  a(n,3)  a(n,4)  ...	a(n,n) | bn |
                 //由第1行可以直接通过a(1,1)*x1 = b1计算出x1 = b1 / a(1,1)
                 //然后向下依次代入每个方程即可求出x的唯一解x = (x1, x2, x3,
                 //..., xn) 这个过程称为正向代入
  for (int i = 0; i < n; ++i) {
    x[i] = e.m_m[i][n] / e.m_m[i][i];
    for (int j = i; j < n; ++j) {
